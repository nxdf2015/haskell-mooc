...
takeFinal n xs  = 
    let size = length xs 
    in
        if size > n then drop (size - n) xs else take size xs
...
palindromify s 
    | isPalindrome s = s
    | otherwise = palindromify (tail (reverse . tail . reverse $ s))
...
addEithers (Left m) _ = Left m
addEithers _  (Left m) = Left m
