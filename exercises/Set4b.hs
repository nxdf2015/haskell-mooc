...
countHelper (Just v ) count = count
countHelper Nothing v = v + 1
...
maxHelper   v max = if v > max  then  v else max 
...
slHelper v (sum, len )  = (sum + v  , len + 1)
