...
member  x (Set (s: set))
  | x == s = True 
  | otherwise = member x (Set set)

add :: Ord a => a -> Set a -> Set a
add x (Set []) = Set [x]
add x (Set (item: set) )
  | x < item = Set (x:item:set)
  | x  == item = Set(item:set)
  | otherwise = case add x (Set set) of
      Set s -> Set (item : s)
...
data State = Start | Error | Finished | Baking | Flour | Sugar | FlourSugar | Mixing |Eggs
  deriving (Eq,Show)
...
reverseNonEmpty (x :| []) = (x :| [])
reverseNonEmpty (x :| ys ) = helperreverseNonEmpty (x :| ys) [] 
  
helperreverseNonEmpty (x:| [] ) acc = ( x :| acc) 
helperreverseNonEmpty ( x :| (y:ys) ) acc = helperreverseNonEmpty (y :| ys)  (x:acc)
...
passwordAllowed [] (MinimumLength x ) =   x == 0
passwordAllowed  password (MinimumLength l) = (length password) >= l
passwordAllowed  []  (DoesNotContain _) = True
passwordAllowed [] (ContainsSome _) = False
 

passwordAllowed (x:xs) (ContainsSome ls) = (elem x ls) || (passwordAllowed xs (ContainsSome ls))
passwordAllowed (x:xs) (DoesNotContain ls) =(notElem x ls) &&  (passwordAllowed xs (DoesNotContain ls))
...
data Arithmetic = Number Integer  | Add Arithmetic Arithmetic  |  Mult Arithmetic Arithmetic 
...
operation "+" e1 e2 = Add e1 e2
operation "*" e1 e2 = Mult e1 e2 
...
render (Mult e1 e2) =  "(" ++ (render e1)  ++ "*" ++   (render e2) ++ ")"