...
study student =
  case student of 
    Freshman -> NthYear 1
    NthYear year-> if year < 7 then  NthYear (year + 1) else Graduated
    Graduated -> Graduated
...
tick :: UpDown -> UpDown
tick (Increase v) = Increase (v + 1)
tick  (Decrease v) = Decrease (v - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Increase v) = Decrease v
toggle (Decrease v) = Increase v
------------------------------------------------------------------------------
...
toList ls = case ls of 
  Empty -> []
  (Pair key value rest) -> (key,value) : (toList rest)
...
fromBin :: Bin -> Int
fromBin End = 0
fromBin (O v) = 2 * (fromBin v)
fromBin (I v) = (2 * (fromBin v)) + 1
...
toBin n 
  | n == 0 = O End
  | n == 1 = I End 
  | (mod n 2) == 1 = I (toBin (div n 2))
  | (mod n 2) == 0 = O (toBin (div n 2))
