...
binomial n k =
    let pred  = n-1
    in (binomial pred k) +  (binomial pred (k-1)) 
...
oddFactorial n = 
    let 
        ...
        start = if even n then (n - 1) else n
    in
        factorial start
...
biggestPrimeAtMost n =
    let 
        helper 2 = 2
        helper k = if (isPrime k) then k else helper (k - 1)
