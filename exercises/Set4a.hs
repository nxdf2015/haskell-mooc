...
distinct [] = True
distinct xs@(y:ys)  =  all (\ x -> length( filter (== x) xs ) == 1 ) xs
...
longest lss = foldr helper  [] lss 

helper longest v 
    | length longest == length v = if (head  longest) < (head v) then longest else v
    | length longest >  length v = longest
    | otherwise = v 
...
incrementKey key xs = map  (addOne key)  xs
...
freqs xs = foldr (\   k acc  -> if Map.member k acc  then  (Map.alter helperFreq k acc) else (Map.insert k 1 acc) ) Map.empty xs 
...
helperAlter   "+"   amount  (Just v) =  Just (v + amount)
helperAlter "-"   amount  (Just v) =  Just (v - amount) 
helperAlter _ _ Nothing = Nothing

isValid (Just v) amount =  amount > 0 && v >= amount 
isValid _ amount = False

transfer :: String -> String -> Int -> Map.Map String Int -> Map.Map String Int
transfer from to amount bank 
    | (Map.member from bank) && (Map.member to bank) =   
        if (isValid (Map.lookup from bank) amount) 
            then
                Map.alter (helperAlter  "-" amount ) from  (Map.alter (helperAlter "+" amount) to bank )  
            else
                bank    
    | otherwise = bank
...
maxIndex arr = foldl (\ i j  -> if arr ! i  <  arr ! j then j else i ) (head (indices arr))(indices arr)
